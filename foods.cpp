...
public:
    ...
    void addBasicFood(const string& name, const vector<string>& keywords, int calories) {
        ...
        saveDatabase();
        cout << "Basic food '" << name << "' added/updated successfully!\n";
    }
    ...
    void addBasicFoodUI() {
        ...
        getline(cin, name); // ✅ Removed extra cin.ignore()
        ...
        cin.ignore(); // ✅ Keep this here to clear newline before next getline
    ...
    void addCompositeFood(const string& name, const vector<string>& keywords, unordered_map<string, int>& ingredients) {
        ...
        saveDatabase();
        cout << "Composite food '" << name << "' added/updated successfully!\n";
    }
...
public:
    ...
    void undo() {
        if (canUndo()) {
            ...
            undoStack.pop();
            undoCmd();
            // No need to push to redo stack since we'll reimplement
        } else {
    ...
    void redo() {
        if (canRedo()) {
            ...
            redoStack.pop();
            redoCmd();
            // No need to push to undo stack since we'll reimplement
        } else {
